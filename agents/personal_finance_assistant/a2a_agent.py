...
from agents.expense_tracking_assistant import expense_tracking_agent
from personal_finance_assistant.remote_agent_connection import (
...
class A2ACompatibleAgent:
    ...
    def register_agent_card(self, card: AgentCard):
        remote_connection = RemoteAgentConnection(self.httpx_client, card)
    ...
    def create_agent(self) -> Agent:
        return Agent(
            ...
            description="""
            An intelligent agent designed to help users manage their
            personal finances by tracking expenses, analyzing spending
            patterns, providing budgeting advice, and offering actionable
            insights to improve financial well-being
            """,
            instruction=self.root_instruction,
            before_model_callback=self.before_model_callback,
            sub_agents=[expense_tracking_agent],
        )
    ...
    def root_instruction(self, context: ReadonlyContext) -> str:
        return StringUtils.populate_variables(
            ...
            variables={'user_name': 'Akshay Priyadarshi'},
        )
    ...
    async def send_message(
        ...
        if isinstance(response, Message):
            return await convert_parts(task_id.parts, tool_context)
        task: Task = response
...
async def convert_parts(parts: list[Part], tool_context: ToolContext):
    """Converts a list of Part objects into their corresponding
    representations using the provided tool_context.

    Args:
        parts (list[Part]): The list of Part objects to convert.
        tool_context (ToolContext): The context used for conversion, which may
            handle file artifacts and other part types.

    Returns:
        list: A list of converted part representations, such as text, data, or
        file references, depending on the type of each Part.
    """
    rval = []
    for p in parts:
...
async def convert_part(part: Part, tool_context: ToolContext):
    """Converts a single Part object into its corresponding
    representation using the provided tool_context.

    Args:
        part (Part): The Part object to convert.
        tool_context (ToolContext): The context used for conversion, which may
            handle file artifacts and other part types.

    Returns:
        The converted representation of the part, such as text, data, or a file
        reference, depending on the type of the Part.
    """
